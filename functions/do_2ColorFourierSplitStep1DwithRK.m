...
indexf0=find(mesh.f==beam.f0,1);
index2f0=find(mesh.f==beam.f0*pulse2.order,1);
% Lwalkoff=pulse2.t_fwhm/abs(fiber.kGV(indexf0)-fiber.kGV(index2f0)); % Walkoff length for 2 pulses copropagating
Twalkoff=abs(fiber.kGV(indexf0)-fiber.kGV(index2f0)).*mesh.Lz;
...
%             EfF=myfft(EtF,mesh.dt); 
%             EfS=myfft(EtS,mesh.dt);               
            %% Group Velocity GV         
...
%             GV2=-1i.*(2*pi*mesh.f-pulse2.w0).*fiber.kGV(index2f0);
            ...
            %Fundamental
            GVD=-1i.*((2*pi.*mesh.f)-pulse1.w0).^2.*fiber.kGVD(indexf0)./2; 
%             TOD=-1i.*((2*pi.*mesh.f)-pulse1.w0).^3.*fiber.kTOD(indexf0)./6; 
            ...
            EtF=myifft(EfF,mesh);
            %Second Harmonic
            GVD2=-1i.*((2*pi.*mesh.f)-pulse2.w0).^2.*fiber.kGVD(index2f0)./2;
...
%                 xlim([0e14 15e14])
                myplot1('field','envelope','no',mesh.t,EtF,EtS)    
...
% 
    ...
    title(['z=',num2str(m*mesh.dz),'  ','Q1=',num2str(Qout),'  ','dt=',num2str(dt)]);                                  
    pause(0.1)                      
    end
...
%calculate diff. equation for Runge Kutta
    ...
    %SST
        ...
        SPMSST2(abs(SPMSST2)<1e-10.*max(abs(SPMSST2)))=0;%take care of numerical errors which might occur 
        SPMSST=myifft(SPMSST2,mesh);                     
    %Ionization
